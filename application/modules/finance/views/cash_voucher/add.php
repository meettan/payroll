<script>
	$(document).ready(function () {
		...
		$("#newrow").click(function(){
			$("#add").append('<tr><td><select id = "acc_code" name="acc_code[]" class="form-control acc_code" style="width: 100%;" required><option value="">Select</option><?php 
		       foreach($row as $value){
		   				echo "<option value=".$value->acc_code.">".$value->acc_head."</option>";
		   			       }
		  ?>
</select></td><td><input type="text"  id="dc_flg" name="dc_flg[]"class="transparent_tag" style="width: 100%; text-align: center;" value="'+g_flg+'" readonly required></td><td><input type="text" class="form-control amount_cls" style="width: 100%; text-align: right;" id="amt" name="amount[]" required ></td><td><button type="button"class="btn btn-danger" id="removeRow"><i class="fa fa-undo" aria-hidden="true"></i></button></td></tr>');
});
	...
	<div class="wraper">      
		...
		<div class="col-md-6 container form-wraper">
			...
			<form method="POST" action="<?php echo site_url("finance/addCashVoucher")?>" onsubmit="return valid_data()">
				...
				<div class="form-group row">
					...
					<div class="col-sm-7">
						...
						<input type = date  name = "voucher_dt" class="transparent_tag" 
						       value="<?php echo $_SESSION['sys_date'];?>" readonly 
						/>
					...
					<div class="col-sm-1">
						...
						<input type="text" name="voucher_mode" value="CASH" class="transparent_tag" 
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						...
						<select name = "voucher_type" id = "v_type" style="width: 100%;" 
						        onchange="set_dr_cr()" class="form-control" required>
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						...
						<input type="text" name="acc_hd" class="transparent_tag" 
						       value="Cash In Hand" style="width: 85px; display:inline;" readonly
						/>

						<input type="text" id="dc" class="transparent_tag" name="dr_cr_flag" 
						       value="" style="display:inline;" readonly 
						/>
				...
				<table>
					...
					<tbody id="add">
						<tr>
							<td><select  id ="acc_code" name="acc_code[]" class="form-control acc_code" style="width: 100%;" required>
								<option value="">Select</option>
								<?php
									foreach($row as $value){
									echo "<option value=".$value->acc_code.">".$value->acc_head."</option>";
							...
							<td><input type="text" class="transparent_tag" id="dc_flg" name="dc_flg[]" 
								   style="width: 100%; text-align: center;" readonly required></td	>

							<td><input type="text" class="form-control amount_cls" id="amt" name="amount[]" 
								   style="width: 100%; text-align: right;" required></td>
						</tr>
					</tbody>
						<tr>
							<td colspan="3" style="text-align:right;">
							    <strong>Total:</strong> 
							    <input name="tot_amt" type="text" class="transparent_tag" 
							     id="tot_amt" style="text-align:left; color:#c1264d; font-size: 25px; width:16%;" 
							     readonly>
						        </td>
						</tr>
