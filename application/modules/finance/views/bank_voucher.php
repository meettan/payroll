<script>
	$(document).ready(function () {
		...
		$("#newrow").click(function(){
			$("#add").append('<tr><td><select id = "acc_code" class="form-control" name="acc_code[]" class="input_text" style="width: 100%;"><option value="0">Select</option><?php
			...
			<form method="POST" action="<?php echo site_url("finance/f_new_cash_voucher")?>" onsubmit="return valid_data()">
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						...
						<select class="form-control" name = "voucher_type" id = "v_type" onchange="set_dr_cr()" class="input_text">
				...
				<div class="form-group row">
					...
					<label for="trans_dt" class="col-sm-2 col-form-label">Bank A/C:</label>

					<div class="col-sm-10">

						<select name="acc_hd" class="form-control" >
							<option value="0">Select</option>	
							<?php
								foreach($bank as $value){
									echo "<option value='".$value->acc_cd."'>".$value->bank_name."</option>";			 
								}				 			 	
							?>
						</select>
				...
				<div class="form-group row">
					...
					<label for="trans_dt" class="col-sm-2 col-form-label">Instrument No.:</label>
					...
					<label for="trans_dt" class="col-sm-2 col-form-label">Instrument Date:</label>
				...
				<table>
					...
					<tbody id="add">
					<tr>
						<td><select class="form-control" id ="acc_code" name="acc_code[]" class="input_text" style="width: 100%;">
								<option value="0">Select</option>
								<?php
									foreach($row as $value){
										echo "<option value=".$value->acc_code.">".$value->acc_head."</option>";
